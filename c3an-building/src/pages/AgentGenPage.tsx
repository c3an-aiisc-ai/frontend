...
import { AGENT_REGISTRY_AGENTS } from "../constants";
import { readCustomAgents, writeCustomAgents } from "../utils/customAgents";
...
function buildUniqueId(base: string, used: Set<string>): string {
  const root = base.trim() || "custom-agent";
  let candidate = root;
  let counter = 1;
          ...
          <section className="rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold text-slate-600">
                <span className="rounded-full bg-slate-100 px-3 py-1">Agents: {customAgents.length}</span>
                <span className="rounded-full bg-slate-100 px-3 py-1">Inputs: {agentStats.totalInputs}</span>
                <span className="rounded-full bg-slate-100 px-3 py-1">Outputs: {agentStats.totalOutputs}</span>
              </div>
            </div>
