...
class ExportRunArtifacts(Stage[TrainedModel, ExportPaths]):
    ...
    def run(self, inp: TrainedModel, **kwargs) -> ExportPaths:
        ...
        weights_pth = str((base_dir / f"{prefix}fusion_model.pth").resolve())
        model = getattr(inp, "_torch_model", None)
        if model is not None:
            torch.save(model.state_dict(), weights_pth)

        preds_csv = str((base_dir / f"{prefix}fusion_predictions.csv").resolve())
        ...
        if output_dir:
            metrics_json = str((base_dir / f"{prefix}fusion_metrics.{fs._ts()}.json").resolve())
            Path(metrics_json).write_text(json.dumps(metrics.to_dict(), indent=2))
        else:
            metrics_json = fs.write_json("tertiary", "fusion_metrics", metrics.to_dict())
